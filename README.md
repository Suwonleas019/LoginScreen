...
- MessageBox.Show("아이디 또는 패스워드가 잘못 입력되었습니다.", "Error", MessageBoxButtons.OK); 을 사용
- 평상시에는 보이지 않는 로그인 실패 문구가 로그인 실패 시 보이게 출력되고 다시 아이디와 비밀번호를 알맞게 입력하면 사라짐
- 로그인 실패 시 라벨의 문구의 visible 속성을 true 로 바꾸어 출력되게 만들고 로그인 성공 시 false로 바꾸어 다시 사라지게 만든다
## 실행 화면 (과제3)
- 과제3 코드의 실행 스크린샷
...
- 과제 내용
- 패스워드의 글자를 모두 삭제되게 만드는 버튼과 마우스를 누르면 패스워드의 암호화가 풀리고 다시 때면 암호화가 다시 적용됨
- 구현 내용과 기능 설명
- 패스워드의 x버튼을 누르면 패스워스 창에 친 모든 글자가 삭제되게 만듬
- 패스워드의 o버튼을 마우스로 누르면 패스워드 글자의 암호화가 풀리고
- 다시 마우스를 때면 글자의 암호화가 다시 적용된다
## 실행 화면 (과제4)
- 과제4 코드의 실행 스크린샷
